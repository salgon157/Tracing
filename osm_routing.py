...
def add_osm_args(parser) -> None:
    """
    Přidá argument --fresh-osm k argparserové instanci.

    Default = False → použije se preset "stable" → identické chování s dneškem.
    True → preset "current" → URL na portech 5001 / 8081.
    """
    parser.add_argument(
        "--fresh-osm",
        action="store_true",
        help="Použij čerstvou routing instanci (porty 5001/8081, C:\\osrm_current) "
             "místo stabilní (5000/8080, C:\\osrm). Vyžaduje běžící Docker kontejnery "
             "osrm-current a ors-current — viz scripts/start_osrm_current.bat.",
    )
