...
if (-not (Test-Path "_baseline_4f0f879\vrp_solver_lines_v6.py")) {
    Write-Host "Baseline worktree chybi - vytvarim: git worktree add --detach _baseline_4f0f879 4f0f879"
    git worktree add --detach _baseline_4f0f879 4f0f879
}
$stamp = Get-Date -Format "yyyyMMdd_HHmm"
$log = "data\results\overnight_$stamp.log"
New-Item -ItemType Directory -Force -Path "data\results" | Out-Null
("=== overnight regression {0} | budget={1} reps={2} dates={3} ===" -f $stamp, $Budget, $Reps, ($Dates -join ',')) | Tee-Object -FilePath $log
("### start {0}" -f (Get-Date -Format "HH:mm")) | Tee-Object -FilePath $log -Append

python regression_ab.py --baseline-dir _baseline_4f0f879 --dates @Dates --depots @Depots `
